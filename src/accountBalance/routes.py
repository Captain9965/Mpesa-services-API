...
accountBalanceBp = Blueprint("accountBalance", "accBalBp", url_prefix="/accBal")

@accountBalanceBp.route("/", methods = ["POST"])
def make_request():
    ...
    try:
        ...
        data = {
            ...
            "IdentifierType": 4,
            "Initiator" : "Lenny",
            "ResultURL": f"https://{str(config('MPESA_CALLBACK_URL'))}/accBal/resultUrl/",
            "QueueTimeOutURL": f"https://{str(config('MPESA_CALLBACK_URL'))}/accBal/queueTimeoutUrl/",
...
#query timeout url:
@accountBalanceBp.route("/queueTimeoutUrl", methods= ['POST','GET'])
def queueTimeoutUrl():
    request_data = request.get_json()
    print(request_data)
    return jsonify({
...
#resulturl:
@accountBalanceBp.route("/resultUrl", methods= ['GET', 'POST'])
def resutlUrl():
    request_data = request.get_json()
    print(request_data)
    return jsonify({
